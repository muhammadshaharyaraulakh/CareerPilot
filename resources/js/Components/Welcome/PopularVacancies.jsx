    ...
    return (
        <section className="w-full bg-white py-12 lg:py-20 px-4 sm:px-6 lg:px-8 border-t border-gray-100/60 transition-colors">
            <div className="max-w-[1320px] mx-auto">
                {/* Section Title */}
                <h2 className="text-[24px] sm:text-[32px] lg:text-[40px] font-medium leading-tight lg:leading-[48px] text-gray-900 mb-8 sm:mb-12 tracking-tight">
                    Most Popular Vacancies
                </h2>

                {/* Grid of Vacancies */}
                <div className="grid grid-cols-1 min-[576px]:grid-cols-2 lg:grid-cols-4 gap-y-6 sm:gap-y-7 gap-x-6 sm:gap-x-8">
                    {listToDisplay.map((vacancy) => (
                        <div
                            key={vacancy.id}
                            className="flex flex-col space-y-1 group cursor-pointer p-2 rounded-lg -mx-2 hover:bg-gray-50/80 transition-colors duration-150"
                        >
                            <h3 className="text-base font-medium text-gray-900 group-hover:text-[#0A65CC] transition-colors duration-200 self-start">
