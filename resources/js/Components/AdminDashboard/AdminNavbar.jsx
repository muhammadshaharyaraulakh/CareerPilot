...
export default function AdminNavbar({ adminUser = "System Administrator" }) {
    const [searchQuery, setSearchQuery] = useState("");
    const [notificationsOpen, setNotificationsOpen] = useState(false);

    return (
        <header className="w-full bg-white border-b border-[#E4E5E8] py-3 px-4 sm:px-6 lg:px-8 sticky top-0 z-30 shadow-2xs">
            <div className="max-w-[1320px] mx-auto flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4">
                    <Link href="/" className="flex items-center shrink-0">
                        ...
                        >
                            <g clipPath="url(#clip0_admin_logo)">
                                <path
                    ...
                    <span className="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1 bg-[#E8F1FF] text-[#0A65CC] text-xs font-semibold rounded-md border border-[#CEE0F5]">
                ...
                <div className="hidden md:flex items-center flex-1 max-w-[420px] h-[42px] border border-[#E4E5E8] rounded-md bg-[#F8F9FA] px-3 focus-within:border-[#0A65CC] focus-within:bg-white transition-all">
                    <MagnifyingGlassIcon className="w-4 h-4 text-[#767E94] mr-2 shrink-0" />
                    <input
                        type="text"
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        placeholder="Search company, job, user, blog..."
                        className="w-full text-xs text-[#18191C] placeholder-[#9199A3] border-none outline-none focus:ring-0 p-0 bg-transparent"
                    />
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="p-2 rounded-md text-[#5E6670] hover:text-[#0A65CC] hover:bg-gray-100 relative transition-colors cursor-pointer"
                        >
                            <BellIcon className="w-5 h-5" />
                            <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-[#E05151]" />
                        </button>

                        {notificationsOpen && (
                            <div className="absolute right-0 mt-2 w-80 bg-white border border-[#E4E5E8] rounded-xl shadow-lg p-4 z-50">
                                <div className="flex items-center justify-between pb-2 mb-3 border-b border-gray-100">
                                    <h4 className="text-xs font-bold text-[#18191C]">
                                        Pending Admin Alerts
                                    </h4>
                                    <span className="text-[10px] bg-[#E8F1FF] text-[#0A65CC] font-semibold px-2 py-0.5 rounded">
                                        3 New
                                    </span>
                                </div>
                                <div className="flex flex-col gap-2.5 text-xs text-[#5E6670]">
                                    <div className="p-2.5 rounded-lg bg-[#F8F9FA] border border-gray-100">
                                        <p className="font-semibold text-[#18191C]">
                                            New Company Registration
                                        </p>
                                        <p className="text-[11px] text-[#767E94] mt-0.5">
                                            TechCorp requested plan approval
                                        </p>
                                    </div>
                                    <div className="p-2.5 rounded-lg bg-[#F8F9FA] border border-gray-100">
                                        <p className="font-semibold text-[#18191C]">
                                            Pending Blog Submission
                                        </p>
                                        <p className="text-[11px] text-[#767E94] mt-0.5">
                                            20 Cool Fonts post awaiting approval
                                        </p>
                                    </div>
                                    <div className="p-2.5 rounded-lg bg-[#F8F9FA] border border-gray-100">
                                        <p className="font-semibold text-[#18191C]">
                                        ...
                                        <p className="text-[11px] text-[#767E94] mt-0.5">
                                            Remote Developer job flagged for review
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="flex items-center gap-2 pl-2 border-l border-gray-200">
                        <div className="w-8 h-8 rounded-full bg-[#0A65CC] text-white flex items-center justify-center font-bold text-xs shadow-2xs">
                            AD
                        </div>
                        <div className="hidden sm:flex flex-col text-left">
                            <span className="text-xs font-bold text-[#18191C] leading-tight">
                                {adminUser}
                            </span>
                            <span className="text-[10px] text-[#0A65CC] font-semibold">
                                Super Administrator
                            </span>
                        </div>
                    </div>
