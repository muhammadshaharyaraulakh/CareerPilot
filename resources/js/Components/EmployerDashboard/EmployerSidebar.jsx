...
export default function EmployerSidebar({
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-3.5 py-3 text-xs sm:text-sm font-medium transition-all cursor-pointer relative ${
                                    isActive
                                        ? "bg-[#E8F1FF] text-[#0A65CC] font-semibold border-l-4 border-[#0A65CC] rounded-none"
                                        : "hover:bg-[#F1F2F4] hover:text-[#18191C] rounded-none"
                                }`}
                            >
                                <Icon className={`w-5 h-5 shrink-0 ${isActive ? "text-[#0A65CC]" : "text-[#767E94]"}`} />
    ...
    return (
        <>
            ...
            {/* Mobile Drawer (<1024px down to 320px) with Framer Motion */}
            <AnimatePresence>
                {isMobileOpen && (
